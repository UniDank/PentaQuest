 ...
 */
  ...
  const _checkbox_class = 'rpgui-checkbox'
  ...
  function create_checkbox(elem) {
    ...
    const new_checkbox = elem.nextSibling
  ...
  let _curr_dragged = null
  let _curr_dragged_point = null
  let _dragged_z = 1000

  // class name we consider as draggable
  const _draggable_class = 'rpgui-draggable'
  ...
  const _progress_class = 'rpgui-progress'
  ...
  function create_progress(elem) {
    ...
    const starting_val = elem.dataset.value !== undefined ? Number.parseFloat(elem.dataset.value) : 1
  ...
  const _radio_class = 'rpgui-radio'
  ...
  function create_radio(elem) {
    ...
    (function (elem, new_radio) {
      new_radio.addEventListener('click', () => {
        if (!elem.disabled)
  ...
  const _dropdown_class = 'rpgui-dropdown'
  ...
  function create_dropdown(elem) {
    ...
    const arrow_down_prefix = '<label>&#9660;</label> '
  ...
  const _slider_class = 'rpgui-slider'
  ...
  function create_slider(elem) {
    ...
    (function (elem, slider_container, thumb, track, state, right_edge, left_edge) {
      ...
      const min = Number.parseFloat(elem.min)
      const max = Number.parseFloat(elem.max)

      // calculate edges width and track actual width
      const edges_width = right_edge.offsetWidth + left_edge.offsetWidth
      const track_width = track.offsetWidth - edges_width
      ...
      left_edge.addEventListener('mousedown', (e) => {
        set_value(min)
      })
      right_edge.addEventListener('mousedown', (e) => {
        set_value(max)
      })
      left_edge.addEventListener('mousemove', (e) => {
        if (state.mouse_down)
          set_value(min)
      })
      right_edge.addEventListener('mousemove', (e) => {
        if (state.mouse_down)
          set_value(max)
      })
      ...
      function _onchange() {
        ...
        const relative_val = Math.round(Number.parseFloat(elem.value) - min)
